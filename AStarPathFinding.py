...
class AStarPathFinding:
    def __init__(self, agent, environment):
        ...
        self.known_states = np.array([], dtype= State)
        wd= 10000
        for ele in self.environment.STATIC_SPRITES:
            if ele != self.environment.FINISH_LINE:
                dist= math.sqrt(Vector2(ele.rect.center).distance_to(Vector2(self.agent.position)))
                if dist < wd:
                    wd= dist
        ...
        self.discovered_states = np.array([init_state], dtype= State)
    ...
    def get_child_states(self, agent, state):
        ...
        for child in child_dict:
            ...
            if not self.isWallCollided(agent, child_state):
                for ele in self.environment.STATIC_SPRITES:
                    if ele != self.environment.FINISH_LINE:
                        dist= math.sqrt(Vector2(ele.rect.center).distance_to(Vector2(agent.position)))
                        if dist < wd:
                            wd= dist
                child_state.compute_F(H= self.euclidian_distance(child_dict[child]["position"], self.environment.FINISH_LINE.rect.center), 
                                      G= self.euclidian_distance(state.location, child_dict[child]["position"]), WD= wd)
                ...
                if self.isFinalTest(agent, current_state):
                    ...
                    for child in child_states:
                        if child in self.known_states:  #Si l'état est PLEINEMENT connu, passe au suivant | pas d'optimisation possible
                            continue
                        else:                           #Si l'état est PARTIELLEMENT connu, on vérifie si on peut améliorer le chemin en passant par cet état
                        ...
                        if newG < child.G and child.isWallState == False:              #Si le chemin est plus court, en passant par cet état, on met à jour l'état
                            child.G= newG
